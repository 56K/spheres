...
public class GameControl {
	...
	class DragListener extends MouseMotionAdapter implements MouseListener{
		...
		private Ball selectBall(Point p) {
			...
			if(ball!=null)
			{
				if(selectedBalls.isEmpty()) {
					...
					else if(ball.getBallColor().equals(lastBall.getBallColor()) && isNeighbour(ball,lastBall))
					{
						ball.setChoosen(true);
						selectedBalls.push(ball);
						return ball;
		...
		private Point posToLogicalPos(Point displayPos) {
			...
			Point logical = new Point((displayPos.x-25)/50, (displayPos.y-25)/50);
			if(logical.x>5 || logical.y>5 || logical.x<0 || logical.y<0)
				return null;
			return logical;
		}
	...
	public boolean formsRectangle(Ball ball) {
		...
		{
			// wenn der ball selektiert, aber nicht der letzte ball ist, ergibt sich ein rechteck
